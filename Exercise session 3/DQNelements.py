...
class ExperienceReplayBuffer:
    ...
    def __init__(self, maximum_length):
        self.buffer =                         # TODO: Using deque ensures efficient removal of oldest elements
...
class MyNetwork(nn.Module):
    ...
    def __init__(self, input_size, output_size):
        super().__init__()
        self.input_layer =                   # TODO First layer: state -> hidden layer
        self.hidden_layer =                  # TODO Second layer: hidden -> hidden layer
        self.output_layer                    # TODO Output layer: hidden -> Q-values
        self.activation =                    # TODO ReLU activation function for hidden layers

    def forward(self, x):
        """Define forward pass"""
                                       # TODO Apply input layer and ReLU
                                       # TODO Apply hidden layer and ReLU
        return                         # TODO Return Q-values for all actions
...
for episode in range(N_EPISODES):
    ...
    for t in range(MAX_STEPS):
        ...
        # TODO Execute action in environment and get feedback (next state, reward, etc.)
        next_state, reward, terminal, truncated, _ = 
        done = terminal or truncated  # Done is True if episode ends
        total_reward += reward

        # TODO Store the experience (state, action, reward, next state, done) in the buffer
        ...
        if len(buffer) >= BATCH_SIZE:
            ...
            # TODO Compute the target Q-values for the next states
...
while not done:
    env.render()
    action = select_action(state, 1)  # Choose action (epsilon=0, i.e., exploit the policy)

...
theta_nt = theta.copy()
for t in range(T):
    theta_nt = A @ theta_nt + b                                                # TODO: write parameter update
    traj_no_target.append(theta_nt.copy())

traj_no_target = np.array(traj_no_target)
true_solution = np.linalg.inv(np.eye(2) - A) @ b                               # TODO: find fixed point solution
...
for t in range(T):
    ...
    theta_t = (1 - alpha) * theta_t + alpha * (A @ theta_target + b)           # TODO: write parameter update with fixed target
    traj_hard_target.append(theta_t.copy())
    
    # Refresh target network every K steps
    if t % K == 0:
        theta_target = theta_t.copy()                                          # TODO: update the target network
...
for t in range(T):
    ...
    theta_target = (1 - tau) * theta_target + tau * theta_t                    # TODO: update the target network with Polyak averaging

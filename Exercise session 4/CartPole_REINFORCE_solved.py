...
def find_returns(episode_rewards_all, method="vanilla"):
    ...
    for episode_rewards in episode_rewards_all:
        ...
        if method == "vanilla":
            ...
            total_rewards[:] = total_return
            returns_mean.append(total_rewards[0])

        elif method == "reward_to_go":
            reward_to_go = 0.0
            for i in reversed(range(n)):
                reward_to_go = reward_to_go + episode_rewards[i]
                total_rewards[i] = reward_to_go
            returns_mean.append(total_rewards[0])

        elif method == "remove_baseline":
            total_return = np.sum(episode_rewards)
            total_rewards[:] = total_return
            returns_mean.append(total_rewards[0])

            total_rewards -= baseline.sum()
            ...
            for i in reversed(range(n)):
                ...
                total_rewards[i] = reward_to_go - np.sum(baseline[i:])
